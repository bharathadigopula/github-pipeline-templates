...
| `.github/workflows/terraform-oci-bootstrap.yml` | OCI bootstrap plan/apply and immediate migration from local runner state to an OCI Object Storage backend |
...
jobs:
  bootstrap:
    uses: bharathadigopula/github-pipeline-templates/.github/workflows/terraform-oci-bootstrap.yml@v0.2.0
    with:
...
For `plan` and `apply`, the bootstrap root must expose `state_bucket_name`, `object_storage_namespace`, and `region` outputs. `apply` uses the saved plan, generates a temporary `backend "oci"` override after the bucket exists, migrates state with `terraform init -migrate-state`, and verifies the remote state.
